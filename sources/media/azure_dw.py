...
# Dummy data path — used automatically when no DW credentials are configured.
# Generated by scripts/generate_dummy_data.py. Remove once real credentials are in .env.
DUMMY_CSV_PATH = "data/raw/azure_dw_clef_raw.csv"
...
def _fetch_raw(start: str, end: str) -> pd.DataFrame:
    ...
    no_creds = not AZURE_CONNECTION_STRING and not all(
        [AZURE_SERVER, AZURE_DATABASE, AZURE_USERNAME, AZURE_PASSWORD]
    )

    if no_creds:
        if not os.path.exists(DUMMY_CSV_PATH):
            raise ValueError(
                "No Azure DW credentials configured and no dummy CSV found at "
                f"{DUMMY_CSV_PATH}. Run scripts/generate_dummy_data.py to create it, "
                "or set AZURE_CONNECTION_STRING in .env."
            )
        print(f"  [azure_dw] No DW credentials — loading dummy data from {DUMMY_CSV_PATH}")
        df = pd.read_csv(DUMMY_CSV_PATH, parse_dates=["enquiry_date"])
        df = df[
            (df["enquiry_date"] >= pd.to_datetime(start)) &
            (df["enquiry_date"] <= pd.to_datetime(end))
        ].copy()
        validate_schema(df)
